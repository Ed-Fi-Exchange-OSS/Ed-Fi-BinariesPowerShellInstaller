...
Function Restore-Database($db, $dbDestinationName, $backupLocation, $dataFileDestination, $logFileDestination) {
    Echo "Restoring: with file relocation and rename"
    $originDbName = $db.src;
    ...
    echo "$backupLocation$originDbName.bak"
...
Function RunBaseEdFiInstall($environment) {
    ...
    # Binaries Metadata
    $binaries = @(  New-Object PSObject -Property @{ name="Api";
                                                    type="WebApp";
                                                    url="https://www.myget.org/F/ed-fi/api/v2/package/EdFi.Ods.WebApi.EFA/3.3.0";
                                                    iisAuthentication = @{ "anonymousAuthentication" = $true 
                                                                            "windowsAuthentication" = $false
                                                                        }
                                                    appSettingsProduction = @{ "apiStartup:type" = 'SharedInstance' };
                                                    appSettingsSandbox = @{ "apiStartup:type" = 'sandbox' };
                                                    databases = @( New-Object PSObject -Property @{src="EdFi_Admin";dest=""}#all environments
                                                                    # For SharedInstance
                                                                    New-Object PSObject -Property @{src="EdFi_Ods_Minimal_Template";dest="EdFi_Ods";environment="Production"}
                                                                    New-Object PSObject -Property @{src="EdFi_Ods_Minimal_Template";dest="";environment="Sandbox"}
                                                                    New-Object PSObject -Property @{src="EdFi_Ods_Populated_Template";dest="";environment="Sandbox"}
                                                                    New-Object PSObject -Property @{src="EdFi_Security";dest=""}#all environments
                                                                )
                                                    connectionStringsProduction = @{
                    ...
                    New-Object PSObject -Property @{ name="Dbs";
                                                    type="Databases";
                                                    url="http://www.toolwise.net/EdFi v3.3.0 databases with Sample Ext.zip";
                                                }
                    New-Object PSObject -Property @{ name="SandboxAdmin";
                                                    description = "This is the SandboxAdmin tool.";
                                                    type="WebApp";
                                                    environment = "Sandbox";
                                                    url="https://www.myget.org/F/ed-fi/api/v2/package/EdFi.Ods.Admin.Web.EFA/3.3.0"
                                                    iisAuthentication = @{ "anonymousAuthentication" = $true 
                                                                            "windowsAuthentication" = $false
                                                                        }
                                                    connectionStrings = @{
                    ...
                    New-Object PSObject -Property @{
                                                    ...
                                                    type="WebApp";
                                                    url="https://www.myget.org/F/ed-fi/api/v2/package/EdFi.Ods.SwaggerUI.EFA/3.3.0";
                                                    iisAuthentication = @{ "anonymousAuthentication" = $true 
                                                                            "windowsAuthentication" = $false
                                                                        }
                                                    appSettings = @{
                    ...
                    New-Object PSObject -Property @{
                                                    ...
                                                    url="https://www.myget.org/F/ed-fi/api/v2/package/EdFi.ODS.AdminApp.Web/3.3.0";
                                                    iisAuthentication = @{ 
    ...
    foreach ($b in $binaries | Where-Object {($_.environment -eq $environment) -or (!$_.environment)}) {
    ...
    foreach ($b in $binaries | Where-Object {($_.type -eq "WebApp") -and (($_.environment -eq $environment) -or (!$_.environment))}) {
        $appName = $b.name
    ...
    Write-HostStep "Step: IIS Configuring Web.Config appSettings, connectionStrings and logfiles"
    foreach ($b in $binaries | Where-Object {($_.type -eq "WebApp") -and (($_.environment -eq $environment) -or (!$_.environment))}) {
        ...
        if($environment -eq "Production") {
            if($b.appSettingsProduction) { Set-AppSettingsInWebConfig $appPhysicalPath $b.appSettingsProduction }
            if($b.connectionStringsProduction) { Set-ConnectionStringsInWebConfig $appPhysicalPath $b.connectionStringsProduction}
        }
        if($environment -eq "Sandbox") {
            if($b.appSettingsSandbox)       { Set-AppSettingsInWebConfig $appPhysicalPath $b.appSettingsSandbox }
            if($b.connectionStringsSandbox) { Set-ConnectionStringsInWebConfig $appPhysicalPath $b.connectionStringsSandbox}
        }
...
Function Install-EdFiProduction {
    # Used to measure execution time.
    $start_time = Get-Date

    RunBaseEdFiInstall "Production"

    #DONE
    Write-Output "Done... Time taken: $((Get-Date).Subtract($start_time).Seconds) second(s)"
}

Function Install-EdFiSandbox {
    # Used to measure execution time.
    $start_time = Get-Date
    
    RunBaseEdFiInstall "Sandbox"
